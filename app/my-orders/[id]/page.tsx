      ...
      <ProtectedRoute>
        <div className="min-h-screen bg-[#F8F9FA] py-16">
          <div className="max-w-xl mx-auto text-center">
            ...
            <h2 className="text-xl font-bold text-[#1A1A1A] mb-2">{error || 'Order not found'}</h2>
            <Link href="/my-orders" className="text-[#0056B3] hover:underline text-sm font-medium">Back to My Orders</Link>
          </div>
        </div>
      </ProtectedRoute>
    );
    ...
    <ProtectedRoute>
      <div className="min-h-screen bg-[#F8F9FA] py-8">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Breadcrumb */}
          <div className="mb-6">
            <Link href="/my-orders" className="text-sm text-[#0056B3] hover:underline font-medium">
              ← Back to My Orders
            </Link>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                {order.inquiryNumber && (
                  <p className="text-sm text-[#6C757D] mt-1">Inquiry: {order.inquiryNumber}</p>
                )}
                <p className="text-sm text-[#6C757D] mt-1">Created: {formatDate(order.createdAt)}</p>
              </div>
          ...
          {order.status !== 'CANCELLED' && order.status !== 'REFUNDED' && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
              <h2 className="text-sm font-semibold text-[#1A1A1A] mb-4 uppercase tracking-wide">Order Progress</h2>
              <div className="flex items-center gap-2 flex-wrap">
                {STATUS_FLOW.map((step, index) => (
                  <div key={step} className="flex items-center gap-2">
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                      index <= currentStepIndex
                        ? 'bg-[#E8F0FE] text-[#0056B3]'
                        : 'bg-gray-100 text-gray-400'
                    }`}>
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                        index <= currentStepIndex ? 'bg-[#0056B3] text-white' : 'bg-gray-300 text-white'
                      }`}>
                        {index + 1}
                      </span>
                      {STATUS_LABELS[step]}
                    ...
                    {index < STATUS_FLOW.length - 1 && (
                      <div className={`w-6 h-0.5 ${index < currentStepIndex ? 'bg-[#0056B3]' : 'bg-gray-200'}`} />
                    )}
                  </div>
                ))}
              </div>
          ...
          {/* Items */}
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
            <h2 className="text-sm font-semibold text-[#1A1A1A] mb-4 uppercase tracking-wide">Order Items</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 text-[#6C757D] text-xs uppercase tracking-wider">
                ...
                <tbody>
                  {order.items?.map((item, idx) => (
                    <tr key={item.id || idx} className="border-b border-gray-100 last:border-0">
                      ...
                      <td className="py-3 font-mono text-[#6C757D]">{item.productSku || '—'}</td>
                      <td className="py-3 text-right">${(item.unitPrice || 0).toFixed(2)}</td>
                      <td className="py-3 text-center font-medium">{item.quantity}</td>
                      <td className="py-3 text-right font-semibold text-[#1A1A1A]">${(item.totalPrice || (item.unitPrice || 0) * item.quantity).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="border-t-2 border-gray-200">
                    <td colSpan={4} className="py-3 text-right font-semibold text-[#1A1A1A]">Total Amount</td>
                    <td className="py-3 text-right font-bold text-lg text-[#0056B3]">${(order.totalAmount || 0).toFixed(2)}</td>
                  </tr>
                  {order.totalWeight !== undefined && order.totalWeight !== null && (
                    <tr>
                      <td colSpan={4} className="py-1 text-right text-sm text-[#6C757D]">Total Weight</td>
                      <td className="py-1 text-right text-sm text-[#6C757D]">{order.totalWeight} kg</td>
                    </tr>
                  )}
                </tfoot>
              </table>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <h2 className="text-sm font-semibold text-[#1A1A1A] mb-4 uppercase tracking-wide">Payment</h2>
              {order.paymentStatus && (
                <div className="mb-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-[#1A1A1A]">${(order.totalAmount || 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between border-t border-gray-100 pt-2">
                  <span className="text-[#6C757D]">Balance</span>
              ...
              {order.paymentRecords && order.paymentRecords.length > 0 && (
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <h3 className="text-xs font-semibold text-[#6C757D] mb-2 uppercase">Payment History</h3>
                  <div className="space-y-2">
                    {order.paymentRecords.map((pr) => (
                      <div key={pr.id} className="text-xs text-[#6C757D] flex justify-between">
                        <span>{pr.paymentMethod || 'Payment'} {pr.transactionId && `(#${pr.transactionId})`}</span>
                        <span className="font-medium text-[#1A1A1A]">${pr.amount.toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {/* Shipping Info */}
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <h2 className="text-sm font-semibold text-[#1A1A1A] mb-4 uppercase tracking-wide">Shipping</h2>
              ...
              {order.shippingAddress ? (
                <div className="space-y-1.5 text-sm mb-4">
                  <p className="font-medium text-[#1A1A1A]">{order.receiverName}</p>
                  <p className="text-[#6C757D]">{order.receiverPhone}</p>
                  <p className="text-[#6C757D]">
                    {[order.shippingAddress, order.shippingCity, order.shippingRegion, order.shippingCountry].filter(Boolean).join(', ')}
                  </p>
                </div>
              ) : (
                <p className="text-sm text-[#6C757D] mb-4">Shipping details will be updated once your order is processed.</p>
              )}
              {order.carrier && (
                <div className="flex justify-between text-sm py-2 border-t border-gray-100">
                  <span className="text-[#6C757D]">Carrier</span>
                  <span className="font-medium text-[#1A1A1A]">{order.carrier}</span>
                </div>
              )}
              {order.trackingNumber && (
                <div className="flex justify-between text-sm py-2 border-t border-gray-100">
                  <span className="text-[#6C757D]">Tracking No.</span>
                  <span className="font-mono font-medium text-[#1A1A1A]">{order.trackingNumber}</span>
                </div>
              )}
              {order.estimatedShipDate && (
                <div className="flex justify-between text-sm py-2 border-t border-gray-100">
                  <span className="text-[#6C757D]">Est. Ship Date</span>
                  <span className="font-medium text-[#1A1A1A]">{formatDate(order.estimatedShipDate)}</span>
                </div>
              )}
              {order.shipmentRecords && order.shipmentRecords.length > 0 && (
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <h3 className="text-xs font-semibold text-[#6C757D] mb-2 uppercase">Shipment History</h3>
                  <div className="space-y-2">
                    {order.shipmentRecords.map((sr) => (
                      <div key={sr.id} className="text-xs text-[#6C757D]">
                        <div className="flex justify-between">
          ...
          {order.notes && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
              <h2 className="text-sm font-semibold text-[#1A1A1A] mb-2 uppercase tracking-wide">Your Notes</h2>
              <p className="text-sm text-[#6C757D]">{order.notes}</p>
            </div>
          )}
        </div>
      </div>
