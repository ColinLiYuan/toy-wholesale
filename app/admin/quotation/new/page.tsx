...
export default function NewQuotationPage() {
  ...
  const [items, setItems] = useState<QuotationItemForm[]>([
    { productId: 0, productName: '', productSku: '', productImage: '', quantity: 1, unitPrice: 0, notes: '' },
  ]);
  ...
  const loadInitialData = async () => {
    try {
      setLoadingData(true);
      const [distList, prodList] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await quotationAdminService.createQuotation({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-gray-900">新建报价单</h1>
        <Link
