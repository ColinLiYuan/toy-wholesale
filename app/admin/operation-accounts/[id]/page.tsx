...
const PLATFORM_OPTIONS: Record<string, Array<{ value: string; label: string }>> = {
  ...
  EMAIL: [
    { value: 'GMAIL', label: 'Gmail' }, { value: 'OUTLOOK', label: 'Outlook / Hotmail' },
    { value: 'YAHOO', label: 'Yahoo' }, { value: '163', label: '网易163' },
    { value: 'QQ', label: 'QQ邮箱' }, { value: 'ALIYUN_MAIL', label: '阿里云邮箱' },
    { value: 'ZOHO', label: 'Zoho' }, { value: 'YANDEX', label: 'Yandex' },
  ],
  PAYMENT: [
    { value: 'PAYPAL', label: 'PayPal' }, { value: 'STRIPE', label: 'Stripe' },
    { value: 'WISE', label: 'Wise' }, { value: 'PAYONEER', label: 'Payoneer' },
    { value: 'LIANLIAN', label: '连连支付' }, { value: 'WORLDFIRST', label: '万里汇' },
  ],
  B2B_PLATFORM: [
    { value: 'ALIBABA_INTERNATIONAL', label: '阿里巴巴国际站' }, { value: 'ECER', label: 'ECER / 宜选网' },
    { value: 'MADE_IN_CHINA', label: '中国制造网' }, { value: 'GLOBAL_SOURCES', label: 'Global Sources' },
  ],
  LOGISTICS: [
    { value: 'DHL', label: 'DHL' }, { value: 'UPS', label: 'UPS' },
    { value: 'FEDEX', label: 'FedEx' }, { value: 'YUNEXPRESS', label: '云途物流' },
    { value: 'DEX', label: '递四方' },
  ],
  CUSTOMS: [
    { value: 'CHINA_SINGLE_WINDOW', label: '中国国际贸易单一窗口' }, { value: 'OTHER', label: '报关行/其他' },
  ],
  CLOUD_SERVICE: [
    { value: 'ALIYUN_CLOUD', label: '阿里云' }, { value: 'CLOUDFLARE', label: 'Cloudflare' },
    { value: 'AWS', label: 'AWS' }, { value: 'GOOGLE_CLOUD', label: 'Google Cloud' },
  ],
  DOMAIN: [
    { value: 'ALIYUN_CLOUD', label: '阿里云（万网）' }, { value: 'CLOUDFLARE', label: 'Cloudflare Registrar' },
    { value: 'NAMECHEAP', label: 'Namecheap' }, { value: 'GODADDY', label: 'GoDaddy' },
  ],
  SERVER: [
    { value: 'ALIYUN_CLOUD', label: '阿里云 ECS' }, { value: 'AWS', label: 'AWS EC2' },
    { value: 'GOOGLE_CLOUD', label: 'Google Cloud VM' }, { value: 'VERCEL', label: 'Vercel' }, { value: 'NETLIFY', label: 'Netlify' },
  ],
  DATABASE: [
    { value: 'ALIYUN_CLOUD', label: '阿里云 RDS' }, { value: 'AWS', label: 'AWS RDS' }, { value: 'OTHER', label: '自建/其他' },
  ],
  ANALYTICS: [
    { value: 'GOOGLE_ANALYTICS', label: 'GA4' }, { value: 'GOOGLE_SEARCH_CONSOLE', label: 'Search Console' },
    { value: 'BING_WEBMASTER', label: 'Bing Webmaster' }, { value: 'AHREFS', label: 'Ahrefs' }, { value: 'SEMRUSH', label: 'SEMrush' },
  ],
  DEV_TOOL: [
    { value: 'GITHUB', label: 'GitHub' }, { value: 'GITLAB', label: 'GitLab' },
    { value: 'DOCKER', label: 'Docker Hub' }, { value: 'VERCEL', label: 'Vercel' },
    { value: 'NPM', label: 'npm' }, { value: 'FIGMA', label: 'Figma' },
  ],
  OTHER: [
    { value: 'GOOGLE_WORKSPACE', label: 'Google Workspace' }, { value: 'NOTION', label: 'Notion' },
    { value: 'CANVA', label: 'Canva' }, { value: 'OTHER', label: '其他' },
  ],
};
...
export default function EditOperationAccountPage() {
  ...
  const fetchAccount = async () => {
    try {
      setFetching(true);
      const data = await operationAccountService.getAccountById(id);
      setFormData(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await operationAccountService.updateAccount(id, cleanData as OperationAccount);
      alert('账号更新成功！');
      router.push('/admin/operation-accounts');
    } catch (error: unknown) {
      const err = error as { backendMessage?: string; message?: string };
      alert(err.backendMessage || err.message || '更新账号失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const err = error as { backendMessage?: string; message?: string };
      alert(err.backendMessage || err.message || '删除失败');
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">账号标识 *</label>
